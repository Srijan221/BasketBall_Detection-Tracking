...
with open (r'/home/srijan/Downloads/Basketball_detection/config.yaml') as file:
  ...
  folder_holding_yolo_files = config['images_path']
...
for each_yolo_file in os.listdir(folder_holding_yolo_files):
  if each_yolo_file.endswith("txt"):
    ...
    if not image_name == each_yolo_file:
      ...
      # Start the XML file
      with open(each_yolo_file.replace('txt', 'xml'), 'w') as f:
        f.write('<annotation>\n')
        f.write('\t<folder>'+ folder_holding_yolo_files +'</folder>\n')
...
if os.path.exists(folder_holding_yolo_files):
